...
# 1. FORZAMOS EL ENTRENAMIENTO SOBRE LOS CSV EXACTOS DE PYTHON
train_data <- fread("../src/data/processed/train_data_clean.csv")
val_data   <- fread("../src/data/processed/val_data_clean.csv")
test_data  <- copy(val_data) # Utilizamos la validación como test set final

# Mapear el nombre del target a lo que espera este script de R
setnames(train_data, "price_by_night_person", "price_per_person", skip_absent = TRUE)
setnames(val_data, "price_by_night_person", "price_per_person", skip_absent = TRUE)
setnames(test_data, "price_by_night_person", "price_per_person", skip_absent = TRUE)

# Dropear leakage columns tal como lo hace Python
cols_to_drop <- c('searchid', 'date', 'name', 'detail', 'destination_name', 'destination_code', 
...
train_data[, (cols_to_drop) := NULL]
val_data[, (cols_to_drop) := NULL]
test_data[, (cols_to_drop) := NULL]
...
asignar_segmento <- function(dt) {
  ...
  # Adaptado para usar total_amenities que es del dataset consolidado
  dt_copy[starRating >= 4.5 | numberOfRooms > 300 | total_amenities > 15, segmento := "Lujo_Resort"]
