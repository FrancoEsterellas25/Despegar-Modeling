...
def plot_custom_shap_single(shap_values, base_value, feature_names, feature_values, top_n=10, target_mean=None):
    """
...
def render_explainability_page() -> None:
    ...
    try:
        ...
        st.error(f"Error al cargar el set de validación: {e}")
        return
    ...
    features_row_moe = DataService.get_features_matrix(row_pl)
    ...
    with col_s1:
        ...
        fig_stacking = plot_custom_shap_single(shaps_s, global_target_mean, names_s, vals_s, top_n=10)
        st.pyplot(fig_stacking)
        
    with col_s2:
        st.markdown(f"<h4 style='text-align: center;'>Mixture of Experts (MoE)</h4>", unsafe_allow_html=True)
        fig_moe = plot_custom_shap_single(shaps_m, global_target_mean, names_m, vals_m, top_n=10)
        st.pyplot(fig_moe)
